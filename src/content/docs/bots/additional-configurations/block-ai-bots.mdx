...
5. Under **Configurations**, select the edit icon. Choose from:
   - **Block only on pages with ads**: Use this option if you wish to block AI bots only on portions of your site that show ads. Cloudflare automatically detects whether ads are present on a page, and only block on hostnames that contain those ad units.
